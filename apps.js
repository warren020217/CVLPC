...
function api(params={}, timeoutMs=20000) {
  return new Promise(async (resolve,reject)=>{
    ...
    try{
      ...
      if(!response.ok)throw new Error(`RCD API HTTP ${response.status}`);
      let data;
      try{data=JSON.parse(text)}
      catch(_){throw new Error("RCD API returned invalid JSON.")}
      if(data?.result==="error")throw new Error(data.error||data.message||"RCD API returned an error.");
      resolve(data);
    }catch(e){
      reject(e.name==="AbortError"?new Error("RCD API timed out. Please try again."):e);
    }finally{
      clearTimeout(timer);
    }
  });
...
function showDoc(data,target,actions=false){
  ...
  target.innerHTML=`<div class="doc">
    ...
    <div class="actions">
      <div class="two">
        <select id="sec"><option value="">Select Section</option></select>
        <select id="person"><option value="">Select Personnel</option></select>
      </div>
      <textarea id="remarks" rows="2" placeholder="Remarks (optional)"></textarea>
      <div class="two">
        <button class="actionBtn green" id="receive">Receive</button>
        <button class="actionBtn" id="forward">Forward</button>
      </div>
...
async function find(id,target,actions=false){
  ...
  try{
    showDoc(await api({action:"getDocument",id}),target,actions);
  }catch(e){
...
async function loadSections(){
  try{
    const d=await api({action:"getSections"});
    const s=$("sec");
...
async function loadPeople(){
  const s=$("sec"),p=$("person");
  p.innerHTML='<option value="">Select Personnel</option>';
  if(!s.value)return;
  try{
    const d=await api({action:"getPersonnel",section:s.value});
  ...
  try{
    await scanner.start(
      ...
      async text=>{
        ...
        let id=text;
        try{id=new URL(text).searchParams.get("id")||text}catch(_){}
...
async function loadLatestMemos(){
  ...
  list.innerHTML='<div class="loading">Loading latest memos...</div>';
  ...
  try{
    ...
    if(!docs.length){
      list.innerHTML='<div class="emptyMemo">No memo records found.</div>';
      return;
    }

    list.innerHTML=docs.map((x,i)=>`
      <div class="memoItem">
        <div class="memoNumber">${i+1}</div>

        <div class="memoInfo">
          <div class="memoControl">${esc(x.controlRefId||"")}</div>
          <div class="memoSubject">${esc(x.subject||"No subject/title")}</div>
          <div class="memoMeta">
            ${esc(x.dateLogged||"")}
            ${x.routingStatus?` · ${esc(x.routingStatus)}`:""}
          </div>
        </div>
        ...
        <button
          class="memoForward"
          type="button"
          data-forward-id="${esc(x.controlRefId||"")}"
        >Forward</button>
      </div>
    `).join("");
  }catch(e){
    list.innerHTML=`<div class="error">Unable to load latest memos: ${esc(e.message)}</div>`;
  }
}

function openMemoForForward(id){
  id=(id||"").trim();
  if(!id)return;
...
$("refreshMemos")?.addEventListener("click",loadLatestMemos);
...
document.addEventListener("click",e=>{
  ...
  if(e.target.classList.contains("memoForward")){
    openMemoForForward(e.target.dataset.forwardId);
  }
});
